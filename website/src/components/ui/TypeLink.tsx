...
function parseTypeName(type: string): {
  displayName: string
  baseType: string
  isGeneric: boolean
} {
  ...
  if (genericPart) {
    ...
    const simplifiedArgs = splitGenericArgs(innerTypes)
      .map((arg) => {
...
export function TypeLink({ type }: TypeLinkProps) {
  ...
  if (baseType.startsWith("Microsoft.") || baseType.startsWith("System.")) {
    return docUrl ? (
      ...
      <code className="font-mono text-xs text-blue-600 dark:text-blue-400">{displayName}</code>
    )
  }
